<script lang="ts">
  import Resizer from "$lib/components/ui/Resizer.svelte";
  ...
  $: workspace = browser && workspaces && workspaces.get(context) || null;
  $: width = browser && workspace && workspace.inspector.width || 100;
  $: visible = browser && workspace && workspace.inspector.visible || false;
</script>
